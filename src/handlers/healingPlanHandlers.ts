...
import { handleError } from '../utils/errorHandler';import { handleBotError } from '../utils/errorHandler';bot.action(/^healingplan_page_(\d+)$/, async (ctx) => {
  try {
    ...
    await ctx.editMessageText(`🗓️ *Your 21-Day Healing Plan* (Page ${page}/${totalPages})\n\n${days}`, { parse_mode: 'Markdown', reply_markup: healingPlanPaginationKeyboard(page, totalPages).reply_markup });
