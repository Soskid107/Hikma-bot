...
import { findOrCreateUser, getUserHealingGoals, generate21DayPlan } from '../services/userService';
import { healingPlanPaginationKeyboard } from './ui';
import { handleBotError } from '../utils/errorHandler';

bot.action(/^healingplan_page_(\d+)$/, async (ctx) => {
  try {
    ...
    await ctx.editMessageText(
      `🗓️ *Your 21-Day Healing Plan* (Page ${page}/${totalPages})\n\n${days}`, 
      { 
        parse_mode: 'Markdown', 
